...
def get_days_for_range(from_date, to_date):
    all_days = []
...
def get_holidays_list(from_date, to_date):
    ...
    filtered_holidays = [
        ...
        if from_date.date() <= holiday["holiday_date"] <= to_date.date()
    ]
    result = {}
    for holiday in filtered_holidays:
        ...
        result[holiday["parent"]][holiday["holiday_date"].strftime("%Y-%m-%d")] = holiday_type
    return result
...
def get_leave_applications(from_date, to_date):
    ...
    leave_applications = frappe.get_all(
        ...
        fields=["name", "from_date", "to_date", "employee", "leave_type", "half_day", "half_day_date"],
    )
    return leave_applications


def map_employee_data(days, holidays, year, is_admin, selected_department, current_employee, selected_employee, from_date, to_date):
    employee_data = []
    unsorted_employees = get_employees()
    if not unsorted_employees:
        return employee_data

    employees = sorted(unsorted_employees, key=lambda e: e.employee_name)

    if selected_department != "all":
        employees = [e for e in employees if e.department == selected_department]

    if selected_employee != "all":
        employees = [e for e in employees if e.name == selected_employee]

    leave_allocations = get_leave_allocations(year)
    leave_applications = get_leave_applications(from_date, to_date)
    current_user = frappe.session.user

    for employee in employees:
        employee_leave_allocation = next(
            (a for a in leave_allocations if a.employee == employee.name), None
        )
        employee_leave_applications = [
            a for a in leave_applications if a.employee == employee.name
        ]
        ...
        for leave_application in employee_leave_applications:
            ...
            if isinstance(app_from, date) and not isinstance(app_from, datetime):
                app_from = datetime.combine(app_from, datetime.min.time())
            if isinstance(app_to, date) and not isinstance(app_to, datetime):
                app_to = datetime.combine(app_to, datetime.min.time())

            for day in get_days_between(app_from, app_to):
                formatted_day = day.strftime("%Y-%m-%d")
                half_day_date = leave_application.half_day_date
                if isinstance(half_day_date, date) and not isinstance(half_day_date, datetime):
                    half_day_date = datetime.combine(half_day_date, datetime.min.time())
                employee_leave_application_array[formatted_day] = {
                    ...
                    "half_day": (
                        ...
                        and half_day_date
                        and half_day_date.strftime("%Y-%m-%d") == formatted_day
                    ),
                }
...
def map_single_employee_data(employee, leave_allocation, leave_applications, selected_department, days, holidays, is_admin, current_employee, current_user):
    employee_data = {}
    employee_days = []
    used_days = 0
    ...
    if not employee_holiday_list:
        company_holiday_list = frappe.db.get_value("Company", frappe.defaults.get_user_default("Company"), "default_holiday_list")
    ...
    for day in days:
        ...
        day_type = LeaveType(name="DEFAULT", color="#636363", abbreviation="", is_main_leave_type=0, half_day=0)

        ledger_holiday_list = None
        if day_str in leave_applications:
            leave_application_name = leave_applications[day_str]["name"]
            ledger_holiday_list = frappe.db.get_value("Leave Ledger Entry", {"transaction_name": leave_application_name}, "holiday_list")
        ...
        if holiday_list_to_use in holidays and day_str in holidays[holiday_list_to_use]:
            day_type = map_leave_type(leave_type=holidays[holiday_list_to_use][day_str], half_day=0, leave_types=leave_types, can_see_leave_data=1)
            ...
            ledger_holiday_list = frappe.db.get_value("Leave Ledger Entry", {"transaction_name": leave_application_name}, "holiday_list")
            if ledger_holiday_list and ledger_holiday_list in holidays and day_str in holidays[ledger_holiday_list]:
                day_type = map_leave_type(leave_type=holidays[ledger_holiday_list][day_str], half_day=0, leave_types=leave_types, can_see_leave_data=1)
    ...
    employee_data["total_leaves_allocated"] = format_number(get_leave_value(leave_allocation, can_see_leave_data, "total_leaves_allocated"))
    employee_data["new_leaves_allocated"] = format_number(get_leave_value(leave_allocation, can_see_leave_data, "new_leaves_allocated"))
    employee_data["unused_leaves"] = format_number(get_leave_value(leave_allocation, can_see_leave_data, "unused_leaves"))
...
def map_leave_type(leave_type, half_day, leave_types, can_see_leave_data):
    ...
    day_type = LeaveType(name="ABSENCE", color=day_type_mapping["ABSENCE"]["color"], abbreviation=day_type_mapping["ABSENCE"]["abbreviation"], is_main_leave_type=0, half_day=half_day)
...
def get_context(context):
    ...
    selected_employee = frappe.local.request.args.get("employee", "all")
    ...
    from_date_str = frappe.local.request.args.get("from_date", default_from)
    to_date_str = frappe.local.request.args.get("to_date", default_to)
    ...
    context.all_employees = sorted(get_employees(), key=lambda e: e.employee_name)
    context.year = year
    ...
    days = get_days_for_range(from_date, to_date)
    holidays = get_holidays_list(from_date, to_date)
